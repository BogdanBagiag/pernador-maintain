...
export default function PropertyInsurancesSection({ propertyId, canEdit }) {
  ...
  // Documente atasate asigurarilor acestei proprietati (o asigurare poate avea mai multe fisiere)
  const { data: files } = useQuery({
    queryKey: ['property-insurance-files', propertyId, insuranceIds.join(',')],
    queryFn: async () => {
      if (insuranceIds.length === 0) return []
      const { data, error } = await supabase
        .from('property_insurance_files')
        .select('*')
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const payload = {
        ...
        tip: formData.tip.trim() || 'Asigurare locuință',
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['property-insurances', propertyId] })
      queryClient.invalidateQueries({ queryKey: ['all-property-insurances'] })
      handleClose()
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => {
      const { error } = await supabase.from('property_insurances').delete().eq('id', id)
      if (error) throw error
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['property-insurances', propertyId] })
  ...
  const uploadFilesMutation = useMutation({
    mutationFn: async ({ insuranceId, fileList }) => {
      for (const file of fileList) {
        ...
        const filePath = `property-insurances/${propertyId}/${insuranceId}/${fileName}`
        ...
        const { error: dbError } = await supabase.from('property_insurance_files').insert({
    ...
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['property-insurance-files', propertyId] })
  ...
  const deleteFileMutation = useMutation({
    mutationFn: async (file) => {
      ...
      const { error } = await supabase.from('property_insurance_files').delete().eq('id', file.id)
      if (error) throw error
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['property-insurance-files', propertyId] })
          ...
          <div className="flex items-center justify-between mb-2 gap-2">
            ...
            {canEdit && (
              <>
                ...
                <button
                  type="button"
                  onClick={() => document.getElementById(`insurance-files-${item.id}`).click()}
    ...
    <div>
      <div className="flex justify-between items-center mb-3">
        <h4 className="font-medium text-gray-800 flex items-center gap-2">
          <ShieldCheck className="w-4 h-4" /> Asigurări
        </h4>
        {canEdit && (
          <button
            onClick={() => setShowModal(true)}
            className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium rounded-md text-white bg-primary-600 hover:bg-primary-700"
          >
            <Plus className="w-3.5 h-3.5" /> Adaugă asigurare
          </button>
        )}
      </div>
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-gray-400 italic mb-1">Nicio asigurare adăugată.</p>
      ) : (
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="ex: Asigurare locuință, PAD..."
