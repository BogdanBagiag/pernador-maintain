...
import { Ticket, Plus, Calendar, DollarSign, Trash2, AlertTriangle, CheckCircle, ChevronDown, ChevronUp, Download } from 'lucide-react'
...
export default function VehicleVignetteSection({ vehicleId, vignettes, isLoading }) {
  ...
  // Separate active and historical vignettes
  ...
  const addMutation = useMutation({
    mutationFn: async () => {
      ...
      // Check if the vignette is already expired
    ...
    return (
      <div className={`border rounded-lg p-4 ${
        ...
        <div className="flex items-start justify-between mb-3">
          ...
          {canEdit && (
            <button onClick={() => handleDelete(vignette)} className="text-red-400 hover:text-red-600">
        ...
        <div className="space-y-2 text-sm">
          ...
          {vignette.serial_number && (
            <p className="text-gray-600 text-xs">Serie: {vignette.serial_number}</p>
          )}
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4">
            ...
            <div className="relative bg-white rounded-lg shadow-xl max-w-lg w-full p-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">{t('vehicles.addVignette')}</h3>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="date" required value={formData.start_date} onChange={(e) => setFormData({ ...formData, start_date: e.target.value })} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Data Sfârșit *</label>
                    <input type="date" required value={formData.end_date} onChange={(e) => setFormData({ ...formData, end_date: e.target.value })} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm" />
                  </div>
                </div>
                <div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Chitanță</label>
              ...
              <div className="mt-6 flex justify-end space-x-3">
                <button onClick={() => setShowAddModal(false)} className="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">Anulează</button>
