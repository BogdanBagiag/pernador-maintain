...
import { Wrench, Plus, Calendar, DollarSign, MapPin, Trash2, FileText, Download, Gauge } from 'lucide-react'
...
export default function VehicleRepairsSection({ vehicleId, repairs, isLoading }) {
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setInvoiceFile(null)
      setFormData({ repair_type: 'corectiva', title: '', description: '', service_provider: '', service_location: '', repair_date: new Date().toISOString().split('T')[0], mileage_at_repair: '', labor_cost: '', parts_cost: '', total_cost: '', status: 'completed', notes: '' })
      setError('')
    },
            ...
            return (
              <div key={repair.id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50">
                <div className="flex items-start justify-between mb-3">
                  ...
                  {canEdit && (
                    <button onClick={() => handleDelete(repair)} className="text-red-400 hover:text-red-600 ml-4">
                ...
                {repair.notes && (
                  <p className="mt-3 text-xs text-gray-500 italic">{repair.notes}</p>
                )}
              </div>
            )
          })}
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4">
            ...
            <div className="relative bg-white rounded-lg shadow-xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
              <h3 className="text-lg font-medium text-gray-900 mb-4">{t('vehicles.addRepair')}</h3>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="date" required value={formData.repair_date} onChange={(e) => setFormData({ ...formData, repair_date: e.target.value })} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm" />
                  </div>
                  <div>
